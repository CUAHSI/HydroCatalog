...
#Region " Web Form Designer Generated Code "
    ...
    Public Shared myCV As ControlledVocabulary
    ...
    Private Sub Page_Init(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Init
        ...
        myCV = New ControlledVocabulary(strTable, strDate)

        'Retrieve all of the rows that will update the controlled vocabulary:
        columnTable = OpenTable("ODMCV", "SELECT t1.[name] AS name, t2.[name] AS type, t1.length " _
        ...
        myCV.Term_ID = CStr(curColRow.Item(0))
        myCV.Columns = CStr(curColRow.Item(0))
        myCV.TableName = Right(strTable, Len(strTable) - 5)
        myCV.TempTableName = strTable
        myCV.Status = "submitted"
        ...
        myCVTable = OpenTable("ODMCV", "SELECT " _
                                        & myCV.Columns & " " _
                                        & "FROM " _
                                        & myCV.TempTableName & " " _
        ...
        Select Case strAction
            Case "A"
                lblTableName.Text = "Add controlled vocabulary to " & myCV.TableName
                i_count = 1
                ...
                For Each curCell In myCVTable.Columns
            ...
            Case "O"
                lblTableName.Text = "Edit controlled vocabulary in " & myCV.TableName
                ...
                End If
                i_count = 1
                For Each curRow In myCVTable.Rows
                    If i_count = 1 Then
                        For Each curCell In myCVTable.Columns
                            If curCell.ColumnName = "IsGeographic" Then
                                If curRow.Item(i_count - 1) = "True" Then
            ...
            Case "D"
                lblTableName.Text = "Delete controlled vocabulary from " & myCV.TableName
                i_count = 1
                If columnTable.Rows.Count > 0 Then
                    For Each curRow In columnTable.Rows
                        'Data entry section:
        ...
        lblAction.Text = myCV.ActionFlag
        lblSubmitter.Text = myCV.SubmitterName
        lblEmail.Text = "<a href='mailto:" & myCV.SubmitterEmail & "'>" & myCV.SubmitterEmail & "</a>"
        lblEmailAddress.Text = myCV.SubmitterEmail
        lblReason.Text = myCV.Reason
...
#End Region
    ...
    Private Sub btnSubmit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
                ...
                If curControl.GetType.ToString.Equals("System.Web.UI.WebControls.TextBox") And curControl.Visible = True Then
                    ...
                    If Not curTxt Is Nothing Then
                        If curTxt.Visible = True Then
                            If (myCV.TableName = "Units" Or myCV.TableName = "SpatialReferences" Or myCV.TableName = "QualityControlLevels") And i_count = 1 Then
        ...
        ExecuteNonQuery("ODMCV", "INSERT INTO temp_CVTESTER VALUES ('" & FormatForSQL(strUpdateQuery) & "','" & Now & "')")

        'had this term/id already been deleted (are they trying to add one that existed
        'at some point)
        checkTable = OpenTable("ODMCV", "SELECT " & lbl1.Text & " FROM " & strTable _
        ...
        If checkTable.Rows.Count > 0 And strAction = "A" And _
            ...
            EmailNotifications("approval", "10", myCV.TableName, myCV.Term_ID, lblEmailAddress.Text, Session.Item("admin"), myCV.ActionFlag, "duplicate", "", strChangeInfo)

            lblMsg.Text = "The change to ODM Controlled Vocabulary " & myCV.TableName & " is on hold because of an apparent conflict with another Term/ID. You will receive an email explaining further action."
        ...
        Else
            ...
            checkTable = OpenTable("ODMCV", "SELECT " & lbl1.Text & " FROM " & Right(strTable, Len(strTable) - 5) _
                                            & " WHERE " & lbl1.Text & " = " & FormatSQLString(strConflict))
            If checkTable.Rows.Count > 0 And strAction = "A" Then
                EmailNotifications("approval", "10", strTable, strID, lblEmailAddress.Text, Session.Item("admin"), myCV.ActionFlag, "duplicate", "", strChangeInfo)
                ...
                pnlReturn.Visible = True
            Else
                ExecuteNonQuery("ODMCV", strUpdateQuery)
                ...
                EmailNotifications("approval", "10", strTable, strID, lblEmailAddress.Text, Session.Item("admin"), myCV.ActionFlag, "app", txtReason.Text, strChangeInfo)

                lblMsg.Text = "The change to ODM Controlled Vocabulary " & myCV.TableName & " has been made. You will receive a confirmation email shortly."
                lblMsg.ForeColor = Drawing.Color.DarkBlue
                pnlMsg.Visible = True
                pnl1.Visible = False
                pnlReturn.Visible = True
    ...
    Private Sub btnReject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReject.Click
        ...
        EmailNotifications("approval", "10", strTable, strID, lblEmailAddress.Text, Session.Item("admin"), myCV.ActionFlag, "rej", txtReason.Text, strChangeInfo)

        lblMsg.Text = "The change to ODM Controlled Vocabulary " & myCV.TableName & " has been withdrawn. You will receive a confirmation email shortly."
        lblMsg.ForeColor = Drawing.Color.DarkBlue
        pnlMsg.Visible = True
        pnl1.Visible = False
        pnlReturn.Visible = True
